...
pub struct DfsProcessQueue<T> {
    queue : Vec< (u32,Vec<GenericStep<T>>) >
}

impl<T> GenericProcessQueue<T> for DfsProcessQueue<T> {

    fn new() -> DfsProcessQueue<T> {
        DfsProcessQueue{queue:Vec::new()}
    }
